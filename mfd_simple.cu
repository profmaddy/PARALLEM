...
__global__ void CalcMFD(int *mask, double *hv, int *fd, double *fa, double* Slopes, int *ok, unsigned int *progressd,  int* localOK, int gridCols, int gridRows, double *weights)
{
    int self;
    double accum;
    ...
    {
        if (!ok[nie]) return;
        accum += fa[nie];// * Slopes[nie + 6];// flowing west
    }
    ...
    {
        if (!ok[nise]) return;
        accum += fa[nise];// * Slopes[nise + 7];;// flowing northwest
    }
    ...
    {
        if (!ok[nis]) return;
        accum += fa[nis];// * Slopes[nis];//flowing north
    }
    ...
    {
        if (!ok[nisw]) return;
        accum += fa[nisw];// * Slopes[nisw + 1];//flowing northeast
    }
    ...
    {
        if (!ok[niw]) return;
        accum += fa[niw];// * Slopes[niw + 2];// flowing east
    }
    ...
    {
        if (!ok[ninw]) return;
        accum += fa[ninw];// * Slopes[ninw + 3]; //flowing southeast
    }
    ...
    {
        if (!ok[nin]) return;
        accum += fa[nin] * Slopes[nin + 4];//flowing south
    }
    ...
    {
        if (!ok[nine]) return;
        accum += fa[nine] * Slopes[nine + 5];//flowing southwest
    }
  ...
  do {
	      ...
	      //copy grids to GPU
	        ...
	        do {
	        		...
	        		CalcMFD<<<dimGrid, dimBlock>>>(device->mask, device->dem, device->fd, device->fa, device->Slopes, ok, progressd, localOK_d, gridColumns, gridRows, device->runoffweight);
