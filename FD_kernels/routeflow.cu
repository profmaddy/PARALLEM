...
// #endif
	...
	shortest_paths_plateaus_initMFD<<<dimGrid, dimBlock>>>(device->mask, device->dem, device->fd, device->SFD, device->shortest_paths, csize /*data->mapInfo.cellsize*/, data->mapInfo.width, data->mapInfo.height, device->lowHeight);
	...
	for (int j = 0; j < fullsize; j++){
			if ((data->mask[j] == 1))
				  {
					 	if (data->shortest_paths[j] > 0) numsfdpath ++;
				  }
		}
